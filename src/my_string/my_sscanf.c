...
int my_sscanf(char *str, const char *format, ...) {
  ...
  if (format) {
    ...
    int format_len = my_strlen(format), writen = 0;
...
int read_format_sscanf(const char *format, Format_sscanf *input) {
  ...
  for (; !my_strchr(specificator, format[i]); i++) {
    if (format[i] == '*') {
      input->asterisk = 1;
    } else if (my_strchr(nums, format[i])) {
      char number[10];
      int j = 0;
      for (; my_strchr(nums, format[i]) && j < 10; i++, j++) {
        number[j] = format[i];
      }
      number[j] = '\0';
      i--;
...
int write_string(va_list args, char *str, Format_sscanf input, int *writen) {
  ...
  while (!my_strchr(" \n\t\r\f", str[read]) && str[read] && read != input.width)
    read++;
  if (buffer) {
    (*writen)++;
...
int write_signed_int(va_list args, const char *str, Format_sscanf input,
                     int *writen) {
  int *va_int = MY_NULL, read = 0;
  long int *va_long = MY_NULL;
  short int *va_short = MY_NULL;
  char *number = calloc(my_strlen(str) + 1, sizeof(char));
  ...
  if (number) {
    ...
    while (my_strchr(nums, str[read]) && str[read] && read != input.width)
      read++;
    my_strncat(number, str, read);
    if (!input.asterisk) {
      if (len_desc == 'h')
    ...
    if ((va_short || va_int || va_long) && read) {
      if (len_desc == 'h')
        *va_short = my_atoi(number, base);
      else if (len_desc == 'l' || len_desc == 'L')
        *va_long = my_atoi(number, base);
      else
        *va_int = my_atoi(number, base);
      (*writen)++;
    }
    free(number);
  }
...
int write_unsigned_int(va_list args, char *str, Format_sscanf input,
                       int *writen) {
  int read = 0;
  unsigned short int *va_unsigned_short = MY_NULL;
  unsigned int *va_unsigned = MY_NULL;
  unsigned long int *va_unsigned_long = MY_NULL;
  char *number = calloc(my_strlen(str) + 1, sizeof(char));
  if (number) {
    char spec = input.specificator, len_desc = input.length_description;
    ...
    if ((va_unsigned_short || va_unsigned || va_unsigned_long) && read) {
      ...
      if (spec == 'p')
        *va_unsigned_long = my_atoi(number, base);
      else if (len_desc == 'h')
        *va_unsigned_short = my_atoi(number, base);
      else if (len_desc == 'l' || len_desc == 'L')
        *va_unsigned_long = my_atoi(number, base);
      else
        *va_unsigned = my_atoi(number, base);
      (*writen)++;
    }
    free(number);
  }
...
int parse_float(va_list args, char *str, Format_sscanf input, int *writen) {
  int read = 0, str_len = my_strlen(str);
  ...
  if (number) {
    ...
    if (str[read] == 'n' || str[read] == 'N' || str[read] == 'i' ||
      ...
      while (my_strchr("NAIFnaif", str[read]) && str[read] &&
             read != input.width && read < nan_width)
        read++;
    } else {
      int decimal_found = 0;
      while ((my_strchr(nums, str[read]) || str[read] == '.') && str[read] &&
      ...
      if (str[read] == 'e' || str[read] == 'E') {
        read++;
        if (str[read] == '+' || str[read] == '-') read++;
        while (my_strchr(nums, str[read]) && str[read]) read++;
      }
    }
    my_strncat(number, str, read);
    if (read == 1 && !my_strchr(nums, number[0])) number[0] = '\0';
...
void write_float(va_list args, char *number, Format_sscanf input, int *writen) {
  float *va_float = MY_NULL;
  double *va_double = MY_NULL;
  long double *va_long_double = MY_NULL;
  if (!input.asterisk) {
  ...
  if ((va_float || va_double || va_long_double) && number[0]) {
    long double result = my_atof(number);
    if (input.length_description == 'l')
      *va_double = result;
    else if (input.length_description == 'L')
