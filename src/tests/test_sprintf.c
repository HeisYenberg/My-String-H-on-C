...
START_TEST(sprintf_c_1) {
  ...
  char a = 'H', b = 'W', c = '!';
  my_sprintf(custom, format, a, b, c);
...
START_TEST(sprintf_c_3) {
  ...
  char a = 'H', b = 'U', c = '!', d = '?';
  my_sprintf(custom, format, a, b, c, d);
...
START_TEST(sprintf_c_5) {
  ...
  my_sprintf(custom, format, a, b, c, d, e);
...
START_TEST(sprintf_percent) {
  ...
  int a = 0, b = 0;
  my_sprintf(custom, format, a);
  sprintf(original, format, b);
  ck_assert_str_eq(custom, original);
}

Suite *test_sprintf() {
  Suite *s = suite_create("\033[42m-=my_sprintf=-\033[0m");
  TCase *tc = tcase_create("my_sprintf");
